...
## 🧭 日常抓取典型示例（榜单 / Report / 详情 / Rank）

下面这些是最常用、最稳定的 MCP 对话模板，适合每天固定使用：

### 1) 看今日榜单（跨平台）

**可直接问：**

- "给我今天全平台最新榜单，按热度排序，先看前 20 条"
- "只看微博、知乎、抖音的当前榜单，各返回前 10 条"
...
- "生成今天的热点摘要报告，按关键词分组，突出前 10 条"
- "生成最近 7 天周报，包含平台对比和趋势变化"
...
- "搜索‘华为 鸿蒙’，返回最相关的 5 条并附链接"
- "读取这条新闻详情（含正文）"

**常用工具：**

- `search_news(include_url=True)`
- `read_article`
- `read_articles_batch`（批量最多 5 篇）

### 4) 看排名变化（Rank 轨迹）

**可直接问：**

- "分析‘特斯拉’相关新闻近 24 小时排名变化"
- "找出今天从 20 名内上升到前 5 的新闻"

**常用工具：**

- `analyze_topic_trend`（含热度/生命周期/趋势）
- `aggregate_news`（跨平台去重后再看排名表现）
- `get_news_by_date`（按时间范围拉取基础数据）

### 5) 先抓取再分析（每日固定流程）

**可直接问：**

- "先触发一次抓取，再给我今日榜单前 20 和日报摘要"
...
> 建议：如果你每天固定跑流程，可统一使用一句话模板：  
> "先抓取最新数据，然后输出今日榜单前20、关键词聚类、上升最快新闻和简报结论。"
