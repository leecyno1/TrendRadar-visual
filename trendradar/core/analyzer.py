...
def count_word_frequency(
    ...
    always_include_top_n: int = 0,
    always_include_group_name: str = "",
    always_include_only_unmatched: bool = True,
) -> Tuple[List[Dict], int]:
    """
    ...
    Args:
        ...
        convert_time_func: 时间格式转换函数
        min_title_length: 标题最小长度（少于该长度则忽略；0=关闭）
        always_include_top_n: 强制收录“综合 TopN”（0=关闭）
        always_include_group_name: 综合分组显示名（可选）
        always_include_only_unmatched: 是否只收录“未命中词组”的新闻
    ...
    # 兜底：强制收录全量合并 TopN（默认关闭）
    try:
        always_include_top_n = int(always_include_top_n or 0)
    except Exception:
        always_include_top_n = 0
    always_include_top_n = max(0, always_include_top_n)
    always_include_group_name = (always_include_group_name or "").strip()

    try:
        min_title_length = int(min_title_length or 0)
    except Exception:
        min_title_length = 0
    min_title_length = max(0, min_title_length)
    ...
    processed_titles = {}
    matched_new_count = 0
    always_include_candidates: List[Dict] = []
    always_include_seen = set()
    ...
    for source_id, titles_data in results_to_process.items():
        ...
        for title, title_data in titles_data.items():
            ...
            if min_title_length > 0:
                ...
                title_len = len("".join(title_s.split()))
            ...
            # 兜底收录：在不改变原有筛选逻辑的前提下，额外收集候选列表
            if always_include_top_n > 0:
                include_this = (not always_include_only_unmatched) or (not matches_frequency_words)
                if include_this:
                    key = (source_id, title)
                    if key not in always_include_seen:
                        always_include_seen.add(key)
                        always_include_candidates.append(
                            _build_title_entry(source_id, title, title_data)
                        )
    ...
    if always_include_top_n > 0 and always_include_candidates:
        sorted_candidates = sorted(
            always_include_candidates,
            key=lambda x: (
                -calculate_news_weight(x, rank_threshold, weight_config),
                min(x["ranks"]) if x.get("ranks") else 999,
                -x.get("count", 1),
            ),
        )
        top_titles = sorted_candidates[:always_include_top_n]
        if top_titles:
            always_included_count = len(top_titles)
            group_name = always_include_group_name or f"综合 Top{len(top_titles)}"
            stats.append(
                {
                    "word": group_name,
                    ...
                    "is_fallback": True,
                    "percentage": (
                        round(len(top_titles) / total_titles * 100, 2)
                        if total_titles > 0
                        else 0
                    ),
                }
    ...
    matched_news_count = sum(
        len(stat["titles"])
        for stat in stats
        if stat["count"] > 0 and not stat.get("is_fallback")
    )
    if mode == "daily":
        extra = f"，兜底收录 {always_included_count} 条" if always_included_count else ""
        print(f"频率词过滤后：{matched_news_count} 条新闻匹配（将显示在推送中）{extra}")
