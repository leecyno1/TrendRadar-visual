...
def load_frequency_words(
    ...
    - !词：过滤词，匹配则排除
    - @数字：该词组最多显示的条数
    - [标题]：分组显示名（不参与匹配；注意与 [GLOBAL_FILTER]/[WORD_GROUPS] 区分）
    ...
    for group in word_groups:
        ...
        section_for_group = current_section
        group_title: Optional[str] = None

        # 检查是否为区域标记 / 分组标题
        if lines[0].startswith("[") and lines[0].endswith("]"):
            tag = lines[0][1:-1].strip()
            tag_upper = tag.upper()
            if tag_upper in ("GLOBAL_FILTER", "WORD_GROUPS"):
                current_section = tag_upper
                section_for_group = tag_upper
                lines = lines[1:]  # 移除标记行
            else:
                # 作为展示用分组标题（不参与匹配）
                # 如果刚处理完 GLOBAL_FILTER，允许直接进入词组区（无需显式 [WORD_GROUPS]）
                if section_for_group == "GLOBAL_FILTER":
                    section_for_group = "WORD_GROUPS"
                    current_section = "WORD_GROUPS"
                group_title = tag
                lines = lines[1:]
